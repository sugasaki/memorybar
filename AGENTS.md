...
- **配布と自動アップデート**: `main` への push で GitHub Actions が Universal ビルドを `latest` リリースへ公開する（`.github/workflows/release.yml`）
  ...
  - **`MenuBarExtra` のパネルの高さ**(#41/#55/#71/#68/#78 で5回壊した最重要注意点):
    **触る前に [Wiki: パネルの高さ問題](https://github.com/sugasaki/memorybar/wiki/%E3%83%91%E3%83%8D%E3%83%AB%E3%81%AE%E9%AB%98%E3%81%95%E5%95%8F%E9%A1%8C) を読むこと**(経緯・機構・外乱注入による検証手順)。要点だけ以下に置く:
    **MenuBarExtra はウィンドウの「現在の大きさ」を根のビューに提案し、返った答えを次の大きさとして保持する**(#78 の実測)。ここから全ての規律が導かれる:
    - **根のビューは高さを実測値で固定する**(`frame(height:)`)。提案をそのまま通す作り(素の ScrollView、min/max/ideal だけの frame)だと、システムが保持する古い大きさを**こだまのように反射して固定**され、画面構成の変更で狂った値が入るとウィンドウだけ大きいまま戻らない(#78 = v0.5.13 の症状)。理想サイズが不定だと 10pt で開く(#41 と同根)
    - 測るのは**中身の内容そのもの**。器の ScrollView を測ると縦の固有サイズが無く潰れる(#41)。ScrollView は器としてかぶせてよい(#68: 画面に収まらない分を逃がす)が、高さの出所は常に中身の実測
    - 実測高さは可視領域(からフレーム換算した予算)までに丸め、`didChangeScreenParametersNotification` でも予算を取り直す。上端固定・可視領域内(パネルがはみ出すと下端に届かない)
    - ウィンドウが実測とずれたら `WindowHeightSync` が setFrame で直す(ウィンドウは大きくなる方向にしか自然追随しない: #71)。**同じ高さの再要求は有限回(3回)で引き下がり、外からウィンドウが動いたら数え直し、一致したら記録を消す**(HeightGovernor)。恒久的に諦めると #71/#78 の症状が固定化し、無限に要求するとシステムと毎秒喧嘩する
    - 検証は実機で: ウィンドウの大きさは `CGWindowListCopyWindowInfo` で毎秒採取し、「外から大きさを押し付ける」「可視領域を偽装して戻す」の両方の外乱からの復帰まで見る。目視だけだと echo 固定は「たまたま合っている」状態と区別できない
...
- **レビューは特定のコミットに対して行われる**。修正を push しても、既存のレビューは古いコミットを指したままで、自動では追いかけてくれない
  - 修正後は `gh pr edit <PR番号> --add-reviewer "@copilot"` で**再依頼**する
  - 届いたレビューが修正後のコミットを見ているかを必ず確かめる（見ていなければ「対応済みの指摘」と「新しい指摘」を取り違える）:
    ```sh
    gh api repos/<owner>/<repo>/pulls/<PR番号>/reviews \
      --jq '.[] | "\(.user.login) \(.commit_id[0:7]) \(.state)"'
    gh pr view <PR番号> --json headRefOid --jq '.headRefOid[0:7]'   # これと一致するか
    ```
  - 実際に、独立レビュー2件が修正前のコミットを対象にしており、既に直した内容を指摘していたことがある
