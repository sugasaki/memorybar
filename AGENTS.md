...
- 残容量 = 未使用（free）+ キャッシュされたファイル（external + purgeable）。使用済み = 物理メモリ − 残容量
- **アクティビティモニタの「使用済みメモリ」は3内訳（アプリ+確保済み+圧縮）の合計ではない**。約0.5〜0.7GB大きく、差分はVMがどの内訳にも計上しないページ。これを「その他」として表示する（Issue #24 で同時採取により0.001GB精度で確認済み）
