...
- **配布と自動アップデート**: `main` への push で GitHub Actions が Universal ビルドを `latest` リリースへ公開する（`.github/workflows/release.yml`）
  ...
  - **`MenuBarExtra` のパネルの高さ**: **測る対象は必ず内容そのもの**にする。`ScrollView` を測ると縦の固有サイズを持たないため潰れる(#41)し、ウィンドウと内容が別々に決まって差分が露出する(#55)。ScrollView を*器としてかぶせる*のは可(#68: 画面に収まらない分を逃がすため)だが、その場合も実測するのは中の内容側。
    ...
    - 測るのは**内容そのもの**の高さ。器の ScrollView ではなく、その中身に `GeometryReader` を当てる。下限を設けて、測定が壊れたときに窓を潰さない
    - 同じ高さを繰り返し要求しない(AppKit が受け付けない場合に無限に往復する)。ただし**一度一致したら記録を消す**。消さないと2回目の折りたたみが効かず #71 に戻る
    - 上端を固定し、可視領域からはみ出させない。丸めた分は器の ScrollView が引き受ける(#68。この器が無いと、低い画面で下端の「終了」に手が届かなくなる)
