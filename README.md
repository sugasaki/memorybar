...
```
残容量        = 未使用(free) + キャッシュされたファイル(external + purgeable)
使用済みメモリ = 物理メモリ(hw.memsize) − 残容量
```

**解放できる領域を残容量として先に定め、使用済みをそこから導出する。** こうすると両者の合計が必ず物理メモリに一致する。ファイルキャッシュは必要に応じて解放されるため、実際に使える残量に含まれる。

注意すべき点として、**アクティビティモニタの「使用済みメモリ」は、その右に並ぶ3つの内訳(アプリ+確保済み+圧縮)の合計ではない**。実測では約0.5〜0.7GB大きい。差分はVMがどの内訳にも計上していないページ(カーネル/ファームウェア予約領域や圧縮機構自身のオーバーヘッド)で、TrueMemはこれを「その他」として表示するため、画面上で内訳の合計と使用済みが一致する。

この式はアクティビティモニタと同時刻に採取して照合し、**0.001GB精度で一致**することを確認している(詳細は [Issue #24](https://github.com/sugasaki/truemem/issues/24))。
